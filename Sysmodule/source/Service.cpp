...
MainService::MainService() {
    ...
    this->exit_ = !Socket::createListeningSocket();
...
void MainService::process() {
    while (!this->exit_) {
        // Keep checking if a client has connected
        if (!Socket::haveConnection()) {
            Socket::acceptConnection();
        } else {
            // If we have a connection keep reading messages
            std::string msg = Socket::readData();
            if (msg.length() > 0) {
